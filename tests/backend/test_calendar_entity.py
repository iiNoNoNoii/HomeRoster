...
from custom_components.homeroster.calendar import (
    HomeRosterCalendar,
    HomeRosterPersonCalendar,
)
...
async def test_main_calendar_entity_id_and_features(coordinator):
    ...
    assert entity.entity_id == "calendar.homeroster"
...
async def test_person_calendar_entity_id_uses_slug(coordinator):
    ...
    entity = HomeRosterPersonCalendar(coordinator, coordinator.entry, anna)
    assert entity.entity_id == "calendar.homeroster_anna_muller"
...
async def test_event_shared_by_multiple_people_appears_in_both_person_calendars(coordinator):
    ...
    for person in (anna, tom):
        entity = HomeRosterPersonCalendar(coordinator, coordinator.entry, person)
        events = await entity.async_get_events(
            coordinator.hass,
