...
| Funktioniert auf einem Gerät/Browser, zeigt aber auf einem anderen „Custom element not found“ (z. B. auf einem Handy in der Begleit-App, während es am Desktop-Browser funktioniert) | Die Karte lädt über ein kleines Lade-Skript, das sich bei einem fehlgeschlagenen Abruf der eigentlichen Kartendatei selbst automatisch wiederholt (5 Versuche mit steigender Wartezeit) – das deckt die mit Abstand häufigste Ursache, eine kurz fehlgeschlagene Anfrage über eine instabile Verbindung (z. B. Reverse-Proxy/Tunnel), automatisch ab. Tritt es danach immer noch auf: Zuerst prüfen, ob die Karte wirklich nicht geladen wurde: Browser-Konsole auf dem betroffenen Gerät öffnen und `customElements.get('homeroster-card')` eingeben. Kommt dabei eine Funktion/Klasse zurück, ist die Karte tatsächlich geladen und registriert – die Fehlermeldung war nur eine veraltete Anzeige von vor einem abgeschlossenen Reconnect im Hintergrund, ein einfaches Neuladen des Dashboards genügt. Kommt `undefined` zurück, hat dieses Gerät das Skript wirklich nie empfangen (oder ausgeführt) – fast immer ein clientseitiges Caching-Problem, kein serverseitiges: Home Assistant liefert jedes zusätzliche Frontend-Modul (unseres wie das aller anderen – HACS-Plugins, `browser_mod` usw. nutzen exakt denselben `import(...).catch(...)`-Mechanismus) unabhängig von Konto oder Gerät identisch aus, und Home Assistants eigenes Frontend umhüllt jeden dieser Imports bereits mit einer eigenen Recovery-/Stale-Build-Behandlung; was übrig bleibt, ist fast immer eine lokal zwischengespeicherte, veraltete Antwort auf genau diesem einen Gerät, die noch nicht verdrängt wurde – am häufigsten direkt nach einem Umbenennen, Update oder einer Neuinstallation der Integration, oder über eine unzuverlässige externe Verbindung (Reverse-Proxy/Tunnel), bei der eine Anfrage kurz fehlgeschlagen ist und zwischengespeichert wurde. Lösungen, von einfach nach zuverlässig: **(1)** Begleit-App vollständig schließen und neu öffnen (nicht nur in den Hintergrund legen), bzw. Browser-Tab per Hard-Refresh neu laden (Strg/Cmd+Shift+R). **(2)** In der App: Einstellungen → Begleit-App → Fehlerbehebung (Bezeichnung je nach App-Version unterschiedlich) → nach einer „Zurücksetzen“-/„Frontend neu laden“-Aktion suchen. **(3)** Zuverlässiger Fallback: im Desktop-Browser DevTools → Application → Service Workers → Unregister, danach Storage → Clear site data; unter Android Telefon-Einstellungen → Apps → Home Assistant → Speicher → **Cache leeren** (nicht „Daten löschen“ – das würde dich ausloggen). Danach das Dashboard neu öffnen. |
