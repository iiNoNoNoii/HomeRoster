...
| It works on one device/browser but shows "Custom element not found" on another (e.g. a phone in the Companion App, while it's fine on a desktop browser) | The card loads through a small loader script that retries itself (5 attempts with backoff) if the actual card file fails to fetch – covering the single most common cause, a dropped request over an unreliable connection (e.g. a reverse proxy/tunnel), automatically. If it still doesn't show up after that: first check whether the card actually failed to load at all – open the browser console on the affected device and run `customElements.get('homeroster-card')`. If that returns a function/class, the card *is* loaded and registered – the error was a stale on-screen message from before a background reconnect finished, and simply reloading the dashboard clears it. If it returns `undefined`, that device's browser genuinely never received (or executed) the script – almost always a client-side caching issue, not a server-side one: Home Assistant serves every extra frontend module (ours and everyone else's – HACS plugins, `browser_mod`, etc. use the exact same `import(...).catch(...)` mechanism) the same way regardless of account or device, and Home Assistant's own frontend already wraps every one of those imports with recovery/stale-build handling; what's left is almost always a locally cached, outdated response on that one device that hasn't been evicted yet – most commonly right after renaming, updating, or reinstalling the integration, or over an unreliable external connection (a reverse proxy/tunnel) where a request briefly failed and got cached. Fixes, in order of ease: **(1)** Fully close and reopen the Companion App (not just background/foreground it), or hard-refresh a browser tab (Ctrl/Cmd+Shift+R). **(2)** In the app: Settings → Companion App → Debugging (wording varies by app version) → look for a "reset"/"reload frontend" action. **(3)** Reliable fallback: in a desktop browser, DevTools → Application → Service Workers → Unregister, then Storage → Clear site data; on Android, phone Settings → Apps → Home Assistant → Storage → **Clear cache** (not "Clear data" – that would log you out). Then reopen the dashboard. |
